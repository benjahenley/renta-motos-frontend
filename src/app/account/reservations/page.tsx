...
import { getToken } from '@/helpers/getToken';
import LoadingScreen from '@/components/loading-screen';
import ReauthRequest from '@/components/errors/reauthRequest'; // Importa el componente de reautenticación
import { useRouter } from 'next/navigation';
import { formatDateToISOWithoutTime } from '@/helpers/formatDate';
import { getReservationsByUserId } from '@/helpers/reservations/getReservationsByUserId';
...
export default function ReservationsPage() {
  ...
  const [searchfilter, setSearchFilter] = useState('');
  const [current, setCurrent] = useState(1);
  const [showReauth, setShowReauth] = useState(false); // Estado para mostrar el componente de reautenticación
  const router = useRouter();

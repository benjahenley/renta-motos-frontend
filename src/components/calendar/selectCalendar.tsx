...
export default function SelectCalendar() {
  ...
  const handleCellClick = (motorboat: Jetski, timeSlot: string) => {
    ...
    const slotStart = new Date(`${formatDateToISOWithoutTime(date)}T${startTime}:00.000Z`).getTime();
    ...
    const isAnyReserved = newSelectedCells.some(cell => isCellReserved(cell.jetskiId, cell.timeSlot));
    if (isAnyReserved) {
      return;
    }
  
    const isAllSelected = newSelectedCells.every(cell => selectedCells.some(
      selectedCell => selectedCell.jetskiId === cell.jetskiId && selectedCell.timeSlot === cell.timeSlot
    ));
  
    if (isAllSelected) {
      setSelectedCells(prevSelectedCells => prevSelectedCells.filter(
        cell => !newSelectedCells.some(newCell => newCell.jetskiId === cell.jetskiId && newCell.timeSlot === cell.timeSlot)
      ));
    } else {
  ...
  const isCellSelected = (jetskiId: number, timeSlot: string) => 
    selectedCells.some(cell => cell.jetskiId === jetskiId && cell.timeSlot === timeSlot);
  ...
  const isCellReserved = (jetskiId: number, timeSlot: string) => {
    ...
    return reservations.some((reservationItem) => {
      ...
      const slotStart = new Date(`${formatDateToISOWithoutTime(date)}T${startTime}:00.000Z`).getTime();
      const slotEnd = new Date(`${formatDateToISOWithoutTime(date)}T${endTime}:00.000Z`).getTime();
      ...
      <div className="m-auto w-full max-w-[496px] rounded-lg border border-gray-200 bg-white p-6 pt-9 sm:p-12">
        {jetskis.length === 0 ? (
          ...
          <>
                      ...
                      {jetskis.map((boat) => (
                        <td
                          ...
                          className={`td px-4 py-2 cursor-pointer ${
                            ...
                            isCellReserved(boat.id, slot) ? 'bg-red-500 cursor-not-allowed' : ''
                          }`}
                          onClick={() =>
                            !isCellReserved(boat.id, slot) && handleCellClick(boat, slot)
                          }
                        ></td>
                      ))}
                    </tr>
            ...
            <div>
              <Button
                disabled={selectedCells.length !== reservation.selected!.adults * (reservation.selected!.rentTime === '4hs' ? 2 : reservation.selected!.rentTime === 'fullDay' ? 4 : 1)}
                size="xl"
                rounded="lg"
                type="submit"
                variant="solid"
